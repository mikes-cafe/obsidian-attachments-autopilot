...
describe("renameTwin", () => {
  ...
  it("renames an existing preview file alongside the twin and updates attachment-prev", async () => {
    const v = new FakeVault();
    await ensureTwin(v, "attachments/old.png", folder);
    // Simulate a previously-generated PNG preview.
    v.binary.set("attachments/twin/preview/old.png.png", new ArrayBuffer(8));
    ...
    v.files.set(
      ...
      [
        ...
        "attachment-type: png",
        'attachment-prev: "[[attachments/twin/preview/old.png.png]]"',
        "---",
        "",
      ].join("\n"),
    );
    ...
    expect(v.binary.has("attachments/twin/preview/new.png.png")).toBe(true);
    const content = v.files.get("attachments/twin/new.png.md")!;
    expect(content).toContain('attachment-ref: "[[attachments/new.png]]"');
    expect(content).toContain(
      'attachment-prev: "[[attachments/twin/preview/new.png.png]]"',
    );
  });
